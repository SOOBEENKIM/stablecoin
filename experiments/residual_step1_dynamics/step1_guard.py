...
def create_lock():
    ...
    for folder in ['residual_dynamics_ml','residual_dynamics_adaptive','residual_economic_validation','residual_nested_validation']:
    ...
    write_new(LOCK,dict(created_utc=now(),environment=environment(),
        ...
        input_sha256={str(f.relative_to(ROOT)):sha(f) for f in list(INPUT_PATHS) + [
            HERE.parent/'residual_nested_validation'/p for p in [
                'LOCK.json','design.json','results/predictions.csv.gz',
                'results/PREDICTIONS_SEALED.json','results/selected_models.json',
                'results/metrics.csv','results/comparisons.csv']]},
