"""Verifier-only v2: disclose numerical-reduction ties; no forecast changes."""
import json
import numpy as np
import pandas as pd
...
def main():
            ...
            for info in INFOS:
                ...
                for fold in sorted(preds.fold.unique()):
                    ...
                    for st in STRATEGIES:
                        ...
                        decision=verify_selection(inner,ids,int(note.candidate))
                        selection_checks.append(dict(horizon=h,variant=v,information=info,fold=fold,strategy=st,**decision))
                        chosen=decision['saved_candidate']
    ...
    write_new(OUT/'SELECTION_REDUCTION_AUDIT.json',selection_checks)
    write_new(OUT/'VERIFICATION.json',dict(**stamp,verifier_amendment=amendment,verified_utc=now(),candidate_streams=streams,
