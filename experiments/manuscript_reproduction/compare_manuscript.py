...
def write_report(cells, meta, df, manifest):
    ...
    lines += [
        ...
        "4. 표 5의 당시 저장 CSV는 원고와 일치한다. 그러나 보존된 kp_robustness.py를 다시 실행하면 CAP 계수 등을 재생성하지 못한다. "
        "점추정 계수는 B를 바꿔도 같으므로 부트스트랩 횟수만으로 이 차이를 설명할 수 없다. 당시 결과를 만든 코드·설정은 추가 확인이 필요하다.",
