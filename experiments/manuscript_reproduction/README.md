...
**추가 원인 추적:** [기본값 재현과 PDF가 다른 이유](difference_audit/RESULTS_KO.md). 불일치 12개 중 10개는 회귀 반복 횟수 및 표별 부트스트랩 횟수 차이로 재현 확인했다. 표 1 왜도와 표 4 p값 각 1개의 작성 경위는 미확인이다.

[워크스테이션 원본 대조](WORKSTATION_SOURCE_CHECK_KO.md): 실제 `stablecoin_v4`의 분석 코드 5개·전처리 CSV·노트북이 이번 재현의 입력과 모두 바이트 단위로 같음을 확인했다. 첨부 최종 원고는 해당 폴더 초안보다 나중에 편집된 버전이다.
