...
근거: [전체 CSV 목록](/home/ssd-990/soobeenkim/stablecoin_audit_20260908/proj_review/csv_inventory.csv), [열·간격 상세](/home/ssd-990/soobeenkim/stablecoin_audit_20260908/proj_review/csv_inventory.json), [노트북 목록](/home/ssd-990/soobeenkim/stablecoin_audit_20260908/proj_review/notebook_inventory.json), [Excel 목록](/home/ssd-990/soobeenkim/stablecoin_audit_20260908/proj_review/xlsx_inventory.json).
...
수집 코드에는 `ticker='KRW-BTC'`, `interval='minute240'`이 명시돼 있다. 저장된 BTC 가격 간격도 대부분 4시간이다. CSV 시각에는 시간대 정보가 없으므로 이를 UTC라고 바로 간주하면 안 된다. [수집 코드](/home/ssd-990/soobeenkim/stablecoin_audit_20260908/proj_review/extracted_notebooks/BTC_PROJ_Team2_final.py:49)
...
근거: [전처리 대조 수치](/home/ssd-990/soobeenkim/stablecoin_audit_20260908/proj_review/preprocessing_checks.json), [재현 스크립트](/home/ssd-990/soobeenkim/stablecoin_audit_20260908/proj_review/verify_preprocessing.py).
...
4. **미래 변동성의 정의를 정확히 구분해야 한다.** `Next_7_Days_Volatility`는 역순 rolling 42로 계산하므로 현재 수익률부터 미래 41개 수익률까지 포함한다. 이를 설명변수로 넣으면 미래정보를 사용하게 된다. 목표변수로 쓰는 경우에는 예측 기준시점과 구간을 명시해야 한다. 한 개의 봉 누락 때문에 모든 42행이 항상 같은 실제 시간을 의미하지도 않는다. [계산 코드](/home/ssd-990/soobeenkim/stablecoin_audit_20260908/proj_review/extracted_notebooks/BTC_PROJ_Team2_final.py:661)
...
6. **final/NEW라는 이름이 단일 최종 모형을 보장하지 않는다.** `BTC_PROJ_Team2_NEW_1.ipynb`의 첫 실험은 `historical_volatility`를 목표로 사용한다. 다른 셀들의 `Next_7_Days_Volatility`와는 다른 대상이다. 학습 2022년 말까지·검증 2023년·테스트 2024년 분할과 학습자료에만 scaler를 적합하는 구현은 확인되지만, 모든 버전이 동일한 실험은 아니다. [NEW_1의 데이터·분할 정의](/home/ssd-990/soobeenkim/stablecoin_audit_20260908/proj_review/extracted_notebooks/BTC_PROJ_Team2_NEW_1.py:25)
7. **전체 노트북의 순차 실행 재현성은 별도로 확인해야 한다.** 예를 들어 final 노트북에는 DataFrame으로 정의한 `train_df`를 `last_obs=train_df-1`에 쓰는 코드가 남아 있다. 이번에는 해당 모델들을 재학습하지 않았으므로 저장된 모든 GARCH/LSTM 결과가 노트북 전체 실행과 일치한다고 인증하지 않는다. [해당 코드](/home/ssd-990/soobeenkim/stablecoin_audit_20260908/proj_review/extracted_notebooks/BTC_PROJ_Team2_final.py:980)
...
검토 당시 원본 213개 파일의 해시는 [source_manifest.json](/home/ssd-990/soobeenkim/stablecoin_audit_20260908/proj_review/source_manifest.json)에 저장했다.
