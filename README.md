...
**2026-09-25 교차 비교 설계:** R(잔차·달력), B(+시장 정보), F(+포지셔닝) 각각에 선형/QRF/분위수 부스팅/문턱형을 적용하도록 설계를 구체화했습니다. 후보마다 자기 과거 오차로 보정하고, 최종 평가와 같은 **보정 후 손실**로 과거3개월에서 설정을 선택합니다. 동일 입력의 알고리즘 비교와 정보 추가 비교, 문턱 기준선 비교를 포함한6개 주 비교를 고정했습니다. **현재는 설계·합성 규칙 검사·자료 가용성 점검까지 완료했으며 새 예측 성능 실험은 실행하지 않았습니다.** 기존 결과는 보존하고 실제 실행 코드는 후속 구현·검증·잠금 대상으로 남깁니다.
